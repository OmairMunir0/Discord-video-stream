...
export async function playStream(
    ...
    function mergeOptions(opts: Partial<PlayStreamOptions>): PlayStreamOptions {
        return {
            ...
            width: isFiniteNonZero(opts.width) && opts.width! > 0 ? Math.round(opts.width!) : defaultOptions.width,
            height: isFiniteNonZero(opts.height) && opts.height! > 0 ? Math.round(opts.height!) : defaultOptions.height,
            frameRate: Math.round(isFiniteNonZero(opts.frameRate) && opts.frameRate! > 0 ? Math.round(opts.frameRate!) : defaultOptions.frameRate),
            readrateInitialBurst: isFiniteNonZero(opts.readrateInitialBurst) && opts.readrateInitialBurst! > 0 ? opts.readrateInitialBurst : defaultOptions.readrateInitialBurst,
        };
    }
